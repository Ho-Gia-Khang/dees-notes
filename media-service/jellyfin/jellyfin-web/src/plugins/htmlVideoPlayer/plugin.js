...
export class HtmlVideoPlayer {
                            ...
                            .then((fontFiles = []) => {
                                fontFiles.forEach((font) => {
                                    const fontUrl = apiClient.getUrl(
                                        `/FallbackFont/Fonts/${encodeURIComponent(
                                            font.Name
                                        )}`,
                                        {
                                            api_key: apiClient.accessToken(),
    ...
    requiresCustomSubtitlesElement() {
        ...
        if (browser.iOS) {
            ...
            // works in the browser but not the native app
            if (
                (userAgent.includes("os 9") || userAgent.includes("os 8")) &&
    ...
    setSubtitleAppearance(elem, innerElem) {
        Promise.all([
            import("../../scripts/settings/userSettings"),
            import(
                "../../components/subtitlesettings/subtitleappearancehelper"
            ),
    ...
    renderTracksEvents(
        ...
        this.fetchSubtitles(track, item).then(function (data) {
            import("../../scripts/settings/userSettings").then(
                (userSettings) => {
                    // show in ui
                    console.debug(
                        `downloaded ${data.TrackEvents.length} track events`
                    );

                    const subtitleAppearance =
                        userSettings.getSubtitleAppearanceSettings();
                    const cueLine = parseInt(
                        subtitleAppearance.verticalPosition,
                        10
                    );
                    ...
                    for (const trackEvent of data.TrackEvents) {
                        ...
                        const cue = new TrackCue(
                            ...
                            normalizeTrackEventText(trackEvent.Text, false)
                        );
