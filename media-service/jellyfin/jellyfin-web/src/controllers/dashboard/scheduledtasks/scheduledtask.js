...
const ScheduledTaskPage = {
    ...
    loadTaskTriggers: function (context, task) {
        ...
        for (let i = 0, length = task.Triggers.length; i < length; i++) {
            ...
            html += "</div>";
            html +=
                '<button class="btnDeleteTrigger" data-index="' +
                i +
                '" type="button" is="paper-icon-button-light" title="' +
    ...
    getTriggerFriendlyName: function (trigger) {
        if (trigger.Type == "DailyTrigger") {
            return globalize.translate(
                "DailyAt",
                ScheduledTaskPage.getDisplayTime(trigger.TimeOfDayTicks)
            );
        }
        ...
        if (
            trigger.Type == "SystemEventTrigger" &&
            trigger.SystemEvent == "WakeFromSleep"
        ) {
    ...
    showAddTriggerPopup: function (view) {
        ...
        view.querySelector("#selectTriggerType").dispatchEvent(
    ...
    confirmDeleteTrigger: function (view, index) {
        confirm(
            globalize.translate("MessageDeleteTaskTrigger"),
            globalize.translate("HeaderDeleteTaskTrigger")
        ).then(function () {
    ...
    deleteTrigger: function (view, index) {
        ...
        ApiClient.getScheduledTask(id).then(function (task) {
            ...
            ApiClient.updateScheduledTaskTriggers(task.Id, task.Triggers).then(
                function () {
...
export default function (view) {
    ...
    view.querySelector(".selectTriggerType").addEventListener(
        "change",
        function () {
    ...
    view.addEventListener("click", function (e) {
        const btnDeleteTrigger = dom.parentWithClass(
            e.target,
            "btnDeleteTrigger"
        );

        if (btnDeleteTrigger) {
            ScheduledTaskPage.confirmDeleteTrigger(
                view,
                parseInt(btnDeleteTrigger.getAttribute("data-index"), 10)
            );
        }
    });
