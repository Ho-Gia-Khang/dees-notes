...
import { getLocale, getLocaleWithSuffix } from "../../../utils/dateFnsLocale";
...
function populateList(page, tasks) {
    ...
    for (let i = 0; i < tasks.length; i++) {
        ...
        if (task.Category != currentCategory) {
            ...
            if (i === 0) {
                html +=
                    '<a is="emby-linkbutton" class="raised button-alt headerHelpButton" target="_blank" href="https://jellyfin.org/docs/general/server/tasks">' +
        ...
        html +=
            '<div class="listItem listItem-border scheduledTaskPaperIconItem" data-status="' +
            task.State +
            '">';
        html +=
            "<a is='emby-linkbutton' style='margin:0;padding:0;' class='clearLink listItemIconContainer' href='/dashboard/tasks/edit?id=" +
            task.Id +
            "'>";
        html +=
            '<span class="material-icons listItemIcon schedule" aria-hidden="true"></span>';
        html += "</a>";
        html += '<div class="listItemBody two-line">';
        const textAlignStyle = globalize.getIsRTL() ? "right" : "left";
        html +=
            "<a class='clearLink' style='margin:0;padding:0;display:block;text-align:" +
            textAlignStyle +
            ";' is='emby-linkbutton' href='/dashboard/tasks/edit?id=" +
            task.Id +
            "'>";
        html += "<h3 class='listItemBodyText'>" + task.Name + "</h3>";
        html +=
            "<div class='secondary listItemBodyText' id='taskProgress" +
            task.Id +
            "'>" +
        ...
        if (task.State === "Running") {
            html +=
                '<button type="button" is="paper-icon-button-light" id="btnTask' +
                task.Id +
                '" class="btnStopTask" data-taskid="' +
                task.Id +
                '" title="' +
            ...
            html +=
                '<button type="button" is="paper-icon-button-light" id="btnTask' +
                task.Id +
                '" class="btnStartTask" data-taskid="' +
                task.Id +
                '" title="' +
                globalize.translate("ButtonStart") +
    ...
    if (task.State === "Idle") {
        if (task.LastExecutionResult) {
            ...
            html += globalize.translate(
                ...
                formatDistanceToNow(endtime, getLocaleWithSuffix()),
        ...
        html +=
            '<div class="taskProgressOuter" title="' +
            progress +
            '%" style="flex-grow:1;">';
        html +=
            '<div class="taskProgressInner" style="width:' + progress + '%;">';
        html += "</div>";
        html += "</div>";
        html +=
            "<span style='color:#00a4dc;margin-left:5px;'>" +
            progress +
            "%</span>";
        html += "</div>";
    } else {
...
export default function (view) {
    function updateTasks(tasks) {
        for (const task of tasks) {
            ...
            if (taskProgress)
                taskProgress.innerHTML = getTaskProgressHtml(task);
