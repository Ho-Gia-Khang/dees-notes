            ...
            actionsheet.show({
                ...
                callback: function (id) {
                    switch (id) {
                        case "open":
                            Dashboard.navigate(
                                "dashboard/devices/edit?id=" + deviceId
                            );
                            break;
    ...
    html += devices
        .map(function (device) {
            ...
            deviceHtml +=
                ...
                "' class='card backdropCard'>";
            deviceHtml += '<div class="cardBox visualCardBox">';
            deviceHtml += '<div class="cardScalable">';
            deviceHtml += '<div class="cardPadder cardPadder-backdrop"></div>';
            deviceHtml += `<a is="emby-linkbutton" href="#/dashboard/devices/edit?id=${escapeHtml(
                device.Id
            )}" class="cardContent cardImageContainer ${getDefaultBackgroundClass()}">`;
            ...
            if (iconUrl) {
                deviceHtml +=
                    ...
                    iconUrl +
                    "');background-size: auto 64%;background-position:center center;\">";
                deviceHtml += "</div>";
            } else {
                deviceHtml +=
                    '<span class="cardImageIcon material-icons tablet_android" aria-hidden="true"></span>';
            }
            ...
            if (canDelete(device.Id)) {
                ...
                deviceHtml +=
                    '<button type="button" is="paper-icon-button-light" data-id="' +
                    escapeHtml(device.Id) +
                    '" title="' +
                    globalize.translate("Menu") +
                    '" class="btnDeviceMenu"><span class="material-icons more_vert" aria-hidden="true"></span></button>';
                deviceHtml += "</div>";
            }
            ...
            if (device.LastUserName) {
                ...
                deviceHtml +=
                    ", " +
                    formatDistanceToNow(
                        Date.parse(device.DateLastActivity),
                        localeWithSuffix
                    );
            }
...
export default function (view) {
    view.querySelector(".devicesList").addEventListener("click", function (e) {
        ...
        if (btnDeviceMenu) {
            showDeviceMenu(
                view,
                btnDeviceMenu,
                btnDeviceMenu.getAttribute("data-id")
            );
        }
    });
    ...
    view.querySelector("#deviceDeleteAll").addEventListener(
        "click",
        function () {
