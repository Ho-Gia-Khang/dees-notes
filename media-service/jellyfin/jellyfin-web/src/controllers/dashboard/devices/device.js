...
function load(page, device, deviceOptions) {
    page.querySelector("#txtCustomName", page).value =
        deviceOptions?.CustomName || "";
...
function loadData() {
    ...
    const device = ApiClient.getJSON(
        ApiClient.getUrl("Devices/Info", {
            Id: id,
        })
    );
    const deviceOptions = ApiClient.getJSON(
        ApiClient.getUrl("Devices/Options", {
            Id: id,
        })
    ).catch(() => undefined);
