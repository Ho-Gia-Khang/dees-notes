...
function showPlaybackInfo(btn, session) {
    ...
    if (displayPlayMethod === "Remux") {
        ...
        if (session.TranscodingInfo?.TranscodeReasons?.length) {
            ...
            session.TranscodingInfo.TranscodeReasons.forEach(function (
                transcodeReason
            ) {
...
function showOptionsMenu(btn, session) {
    import("../../components/actionSheet/actionSheet").then(
        ({ default: actionsheet }) => {
            ...
            if (
                session.ServerId &&
                session.DeviceId !== ServerConnections.deviceId()
            ) {
                menuItems.push({
...
function onActiveDevicesClick(evt) {
    ...
    if (btn) {
        ...
        if (card) {
            ...
            const session = (DashboardPage.sessionsList || []).filter(function (
                dashboardSession
            ) {
            ...
            if (session) {
                if (btn.classList.contains("btnCardOptions")) {
                    ...
                    ServerConnections.getApiClient(
                        session.ServerId
                    ).sendPlayStateCommand(session.Id, "Stop");
                } else if (
                    btn.classList.contains("btnSessionPlayPause") &&
                    session.PlayState
                ) {
                    ServerConnections.getApiClient(
                        session.ServerId
                    ).sendPlayStateCommand(session.Id, "PlayPause");
                }
            }
        }
    }
...
function filterSessions(sessions) {
    ...
    for (let i = 0, length = sessions.length; i < length; i++) {
        ...
        if (
            datetime
                .parseISO8601Date(session.LastActivityDate, true)
                .getTime() >= minActiveDate
        ) {
            list.push(session);
        }
    }
    ...
    apiClient
        ...
        .then(function (result) {
            ...
            if (!result.Items.length) {
                view.querySelector(".activeRecordingsSection").classList.add(
                    "hide"
                );
            ...
            view.querySelector(".activeRecordingsSection").classList.remove(
                "hide"
            );
...
function reloadSystemInfo(view, apiClient) {
    ...
    queryClient
        ...
        .then((systemInfo) => {
            ...
            view.querySelector("#transcodePath").innerText =
                systemInfo.TranscodingTempPath;
            view.querySelector("#metadataPath").innerText =
...
function renderActiveConnections(view, sessions) {
    ...
    for (let i = 0, length = sessions.length; i < length; i++) {
        ...
        if (elem) {
            ...
            const className =
                "scalableCard card activeSession backdropCard backdropCard-scalable";
            ...
            if (imgUrl) {
                html +=
                    '<div class="sessionNowPlayingContent sessionNowPlayingContent-withbackground"';
                html +=
                    ' data-src="' +
                    imgUrl +
                    '" style="display:inline-block;background-image:url(\'' +
                    imgUrl +
                    "');\"></div>";
            } else {
            ...
            html +=
                '<div class="sessionAppName" style="display:inline-block; text-align:left;"  dir="ltr" >';
            html +=
                '<div class="sessionDeviceName">' +
                escapeHtml(session.DeviceName) +
                "</div>";
            html +=
                '<div class="sessionAppSecondaryText">' +
            ...
            html +=
                '<div class="sessionNowPlayingInfo" data-imgsrc="' +
                nowPlayingName.image +
                '">';
            html +=
                '<span class="sessionNowPlayingName">' +
                nowPlayingName.html +
                "</span>";
            html += "</div>";
            html +=
                '<div class="sessionNowPlayingTime">' +
                escapeHtml(DashboardPage.getSessionNowPlayingTime(session)) +
                "</div>";
            html += "</div>";

            let percent =
                (100 * session?.PlayState?.PositionTicks) /
            ...
            html +=
                '<div class="sessionCardButtons flex align-items-center justify-content-center">';
            ...
            let btnCssClass =
                ...
                session.SupportsRemoteControl
                    ? ""
                    : " hide";
            const playIcon = session.PlayState.IsPaused
                ? "play_arrow"
                : "pause";

            html +=
                '<button is="paper-icon-button-light" class="sessionCardButton btnSessionPlayPause paper-icon-button-light ' +
                btnCssClass +
                '"><span class="material-icons ' +
                playIcon +
                '" aria-hidden="true"></span></button>';
            html +=
                '<button is="paper-icon-button-light" class="sessionCardButton btnSessionStop paper-icon-button-light ' +
                btnCssClass +
                '"><span class="material-icons stop" aria-hidden="true"></span></button>';
            html +=
                '<button is="paper-icon-button-light" class="sessionCardButton btnSessionInfo paper-icon-button-light ' +
                btnCssClass +
                '" title="' +
                globalize.translate("ViewPlaybackInfo") +
                '"><span class="material-icons info" aria-hidden="true"></span></button>';

            btnCssClass =
                session.ServerId &&
                session.SupportedCommands.indexOf("DisplayMessage") !== -1 &&
                session.DeviceId !== ServerConnections.deviceId()
                    ? ""
                    : " hide";
            html +=
                '<button is="paper-icon-button-light" class="sessionCardButton btnSessionSendMessage paper-icon-button-light ' +
                btnCssClass +
                '" title="' +
                globalize.translate("SendMessage") +
                '"><span class="material-icons message" aria-hidden="true"></span></button>';
            html += "</div>";
            ...
            html += userImage
                ? '<div class="activitylogUserPhoto" style="background-image:url(\'' +
                  userImage +
                  "');\"></div>"
                : '<div style="height:1.71em;"></div>';
...
function renderRunningTasks(view, tasks) {
    ...
    for (let i = 0, length = tasks.length; i < length; i++) {
        ...
        if (task.State === "Running") {
            ...
            html += "</progress>";
            html +=
                "<span style='color:#00a4dc;margin-left:5px;margin-right:5px;'>" +
                progress +
                "%</span>";
            html +=
                '<button type="button" is="paper-icon-button-light" title="' +
                globalize.translate("ButtonStop") +
                '" onclick="DashboardPage.stopTask(this, \'' +
                task.Id +
                '\');" class="autoSize"><span class="material-icons cancel" aria-hidden="true"></span></button>';
...
window.DashboardPage = {
    ...
    getSessionNowPlayingStreamInfo: function (session) {
        ...
        if (displayPlayMethod === "DirectPlay") {
            ...
            if (session.TranscodingInfo?.Framerate) {
                html += `${globalize.translate("Framerate")}: ${
                    session.TranscodingInfo.Framerate
                }fps`;
            }
    ...
    getSessionNowPlayingTime: function (session) {
        ...
        if (nowPlayingItem) {
            if (session.PlayState.PositionTicks) {
                html += datetime.getDisplayRunningTime(
    ...
    getNowPlayingName: function (session) {
        ...
        if (nowPlayingItem.Artists?.length) {
            ...
            bottomText = topText;
            topText = escapeHtml(
                nowPlayingItem.SeriesName || nowPlayingItem.Album
            );
        ...
        if (imgUrl) {
            topText =
                '<img src="' +
                imgUrl +
                '" style="max-height:24px;max-width:130px;" />';
        }
    ...
    getUsersHtml: function (session) {
        ...
        for (
            let i = 0, length = session.AdditionalUsers.length;
            i < length;
            i++
        ) {
    ...
    updateSession: function (row, session) {
        ...
        if (
            ...
            row.querySelector(".btnSessionSendMessage").classList.remove(
                "hide"
            );
        } else {
        ...
        const btnSessionPlayPauseIcon =
            btnSessionPlayPause.querySelector(".material-icons");
        btnSessionPlayPauseIcon.classList.remove("play_arrow", "pause");
        btnSessionPlayPauseIcon.classList.add(
            session.PlayState?.IsPaused ? "play_arrow" : "pause"
        );

        row.querySelector(".sessionNowPlayingTime").innerText =
            DashboardPage.getSessionNowPlayingTime(session);
        row.querySelector(".sessionUserName").innerHTML =
            DashboardPage.getUsersHtml(session);
        row.querySelector(".sessionAppSecondaryText").innerText =
        ...
        if (
            !(
                nowPlayingName.image &&
                nowPlayingName.image ==
                    nowPlayingInfoElem.getAttribute("data-imgsrc")
            )
        ) {
            nowPlayingInfoElem.innerHTML = nowPlayingName.html;
            nowPlayingInfoElem.setAttribute(
                "data-imgsrc",
                nowPlayingName.image || ""
            );
        }
        ...
        let percent =
            (100 * session?.PlayState?.PositionTicks) /
            nowPlayingItem?.RunTimeTicks;
        playbackProgressElem.outerHTML = indicators.getProgressHtml(
            percent || 0,
            {
                containerClass: "playbackProgress",
            }
        ...
        transcodingProgress.outerHTML = indicators.getProgressHtml(
            percent || 0,
            {
                containerClass: "transcodingProgress",
            }
        ...
        if (imgUrl != imgElem.getAttribute("data-src")) {
            ...
            if (imgUrl) {
                imgElem.classList.add(
                    "sessionNowPlayingContent-withbackground"
                );
                row.querySelector(
                    ".sessionNowPlayingInnerContent"
                ).classList.add("darkenContent");
            } else {
                imgElem.classList.remove(
                    "sessionNowPlayingContent-withbackground"
                );
                row.querySelector(
                    ".sessionNowPlayingInnerContent"
                ).classList.remove("darkenContent");
            }
        }
    },
...
export default function (view) {
    ...
    view.querySelector(".activeDevices").addEventListener(
        "click",
