...
export default function (view) {
    ...
    function setTitle(item, parentName) {
        ...
        if (item.Type == "Movie" && item.ProductionYear) {
            title += ` (${datetime.toLocaleString(item.ProductionYear, {
                useGrouping: false,
            })})`;
    ...
    const _focus = debounce(
        (focusElement) => focusManager.focus(focusElement),
        50
    );
    ...
    nowPlayingPositionSlider.getMarkerInfo = function () {
        ...
        return (
            currentItem?.Chapters?.map((currentChapter) => ({
                ...
                progress:
                    currentChapter.StartPositionTicks /
                    currentItem.RunTimeTicks,
            })) || []
        );
    };
