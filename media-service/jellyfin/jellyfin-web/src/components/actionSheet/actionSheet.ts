...
export function show(options: Options) {
    ...
    if (layoutManager.tv) {
        html += `<button is="paper-icon-button-light" class="btnCloseActionSheet hide-mouse-idle-tv" tabindex="-1" title="${globalize.translate(
            "ButtonBack"
        )}">
    ...
    for (let i = 0; i < options.items.length; i++) {
        ...
        if (item.secondaryText) {
            html += `<div class="listItemBodyText secondary">${escapeHtml(
                item.secondaryText
            )}</div>`;
        }
        ...
        if (item.asideText) {
            html += `<div class="listItemAside actionSheetItemAsideText">${escapeHtml(
                item.asideText
            )}</div>`;
        }
    ...
    if (options.showCancel) {
        ...
        html += `<button is="emby-button" type="button" class="btnCloseActionSheet">${globalize.translate(
            "ButtonCancel"
        )}</button>`;
        html += "</div>";
    }
