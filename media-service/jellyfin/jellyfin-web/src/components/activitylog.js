...
import serverNotifications from "../scripts/serverNotifications.js";
import "../elements/emby-button/emby-button.js";
import "./listview/listview.scss";
import ServerConnections from "./ServerConnections.js";
import alert from "./alert.js";
import { getLocale } from "../utils/dateFnsLocale.js";
...
function getEntryHtml(entry, apiClient) {
    ...
    if (entry.UserId && entry.UserPrimaryImageTag) {
        html +=
            '<span class="listItemIcon material-icons dvr" aria-hidden="true" style="width:2em!important;height:2em!important;padding:0;color:transparent;background-color:' +
            color +
            ";background-image:url('" +
            apiClient.getUserImageUrl(entry.UserId, {
                type: "Primary",
        ...
        html +=
            '<span class="listItemIcon material-icons ' +
            icon +
    ...
    html += formatRelative(Date.parse(entry.Date), Date.now(), {
        locale: getLocale(),
    });
    html += "</div>";
    ...
    if (entry.Overview) {
        html += `<button type="button" is="paper-icon-button-light" class="btnEntryInfo" data-id="${
            entry.Id
        }" title="${globalize.translate("Info")}">
...
function reloadData(instance, elem, apiClient, startIndex, limit) {
    if (startIndex == null) {
        startIndex = parseInt(
            elem.getAttribute("data-activitystartindex") || "0",
            10
        );
    }
    ...
    ApiClient.getJSON(
        ApiClient.getUrl("System/ActivityLog/Entries", {
        ...
        if (!startIndex) {
            const activityContainer = dom.parentWithClass(
                elem,
                "activityContainer"
            );
...
class ActivityLog {
    ...
    destroy() {
        ...
        if (options) {
            ...
            ServerConnections.getApiClient(options.serverId).sendMessage(
