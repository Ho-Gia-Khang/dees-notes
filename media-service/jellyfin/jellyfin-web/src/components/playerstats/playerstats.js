...
function getDisplayTranscodeFps(session, player) {
    ...
    return `${transcodeFramerate} fps (${(
        transcodeFramerate / originalFramerate
    ).toFixed(2)}x)`;
}
...
function getSyncPlayStats() {
    ...
    syncStats.push({
        ...
        value:
            stats.TimeSyncOffset +
            " " +
            globalize.translate("MillisecondsUnit"),
    });
...
function getStats(instance, player) {
    ...
    return Promise.all([statsPromise, sessionPromise]).then(function (
        responses
    ) {
