...
function getImageUrl(item, imageOptions = {}) {
    const url =
        seriesImageUrl(item, imageOptions) || imageUrl(item, imageOptions);
...
function updatePlayerState(player, state, eventName) {
    ...
    if (eventName === "init") {
        // transform "init" event into "timeupdate" to restraint update rate
    ...
    const duration = parseInt(
        item.RunTimeTicks ? item.RunTimeTicks / 10000 : 0,
        10
    );
    const currentTime = parseInt(
        playState.PositionTicks ? playState.PositionTicks / 10000 : 0,
        10
    );
    ...
    if ("mediaSession" in navigator) {
        ...
        const itemImageUrl =
            seriesImageUrl(item, { maxHeight: 3000 }) ||
            imageUrl(item, { maxHeight: 3000 });
...
if ("mediaSession" in navigator) {
    ...
    navigator.mediaSession.setActionHandler("seekto", function (object) {
        const item =
            playbackManager.getPlayerState(currentPlayer).NowPlayingItem;
        // Convert to ms
        const duration = parseInt(
            item.RunTimeTicks ? item.RunTimeTicks / 10000 : 0,
            10
        ...
        playbackManager.seekPercent(
            (wantedTime / duration) * 100,
            currentPlayer
        );
    });
}
