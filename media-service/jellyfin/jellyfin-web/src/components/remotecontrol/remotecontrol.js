...
function getNowPlayingNameHtml(nowPlayingItem, includeNonNameInfo) {
    return nowPlayingHelper
        .getNowPlayingNames(nowPlayingItem, includeNonNameInfo)
        .map(function (i) {
...
function updateNowPlayingInfo(context, state, serverId) {
    ...
    const displayName = item
        ? getNowPlayingNameHtml(item).replace("<br/>", " - ")
        : "";
    if (item) {
            ...
            if (item.Artists != null) {
                if (item.ArtistItems != null) {
                    for (const artist of item.ArtistItems) {
                        artistsSeries += `<a class="button-link" is="emby-linkbutton" href="#/details?id=${
                            artist.Id
                        }&serverId=${nowPlayingServerId}">${escapeHtml(
                            artist.Name
                        )}</a>`;
...
export default function () {
    ...
    function init(ownerView, context) {
        ...
        volumecontrolHtml += `<button is="paper-icon-button-light" class="buttonMute autoSize" title=${globalize.translate(
            "Mute"
        )}><span class="xlargePaperIconButton material-icons volume_up" aria-hidden="true"></span></button>`;
        volumecontrolHtml +=
