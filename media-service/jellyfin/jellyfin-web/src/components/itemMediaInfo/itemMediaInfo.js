...
const copyButtonHtml = layoutManager.tv
    ? ""
    : `<button is="paper-icon-button-light" class="btnCopy" title="${globalize.translate(
          "Copy"
      )}" aria-label="${globalize.translate("Copy")}"
...
function getMediaSourceHtml(user, item, version) {
    ...
    if (version.Name) {
        html += `<div><h2 class="mediaInfoStreamType">${escapeHtml(
            version.Name
        )}${copyButtonHtml}</h2></div>\n`;
    }
    if (version.Container) {
        html += `${createAttribute(
            globalize.translate("MediaInfoContainer"),
            version.Container
        )}<br/>`;
    }
    if (version.Formats?.length) {
        html += `${createAttribute(
            globalize.translate("MediaInfoFormat"),
            version.Formats.join(",")
        )}<br/>`;
    }
    if (version.Path && user?.Policy.IsAdministrator) {
        html += `${createAttribute(
            globalize.translate("MediaInfoPath"),
            version.Path,
            true
        )}<br/>`;
    }
    if (version.Size) {
        ...
        html += `${createAttribute(
            globalize.translate("MediaInfoSize"),
            size
        )}<br/>`;
    }
...
function createAttribute(label, value, isLtr) {
    return `<span class="mediaInfoLabel">${label}</span>${attributeDelimiterHtml}<span class="mediaInfoAttribute" ${
        isLtr && 'dir="ltr"'
