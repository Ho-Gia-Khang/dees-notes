...
function getLandingScreenOptionsHtml(type, userValue) {
    return getLandingScreenOptions(type)
        .map((o) => {
            ...
            return `<option value="${optionValue}"${selectedHtml}>${escapeHtml(
                o.name
            )}</option>`;
        })
        .join("");
...
function renderViewOrder(context, user, result) {
    ...
    html += result.Items.map((view) => {
        ...
        currentHtml += `<button type="button" is="paper-icon-button-light" class="btnViewItemUp btnViewItemMove autoSize" title="${globalize.translate(
            "Up"
        )}"><span class="material-icons keyboard_arrow_up" aria-hidden="true"></span></button>`;
        currentHtml += `<button type="button" is="paper-icon-button-light" class="btnViewItemDown btnViewItemMove autoSize" title="${globalize.translate(
            "Down"
...
function getPerLibrarySettingsHtml(item, user, userSettings) {
    ...
    if (
        ...
        html += "<div>";
        html += "<label>";
        html += `<input type="checkbox" is="emby-checkbox" class="chkIncludeInMyMedia" data-folderid="${
            item.Id
    ...
    if (!excludeFromLatest.includes(item.CollectionType || "")) {
        ...
        html += `<input type="checkbox" is="emby-checkbox" class="chkIncludeInLatest" data-folderid="${
            item.Id
        }"${isChecked ? ' checked="checked"' : ""}/>`;
        html += `<span>${globalize.translate(
    ...
    if (
        ...
        html += `<select is="emby-select" class="selectLanding" data-folderid="${idForLanding}" label="${globalize.translate(
            "LabelDefaultScreen"
        )}">`;
